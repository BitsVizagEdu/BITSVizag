...
export async function load({url}) {
    ...
    if(!result.hasOwnProperty("Student_Name") || result.Student_Name.length === 0) {
        throw redirect(302, "/application-form")
    }

    if(!result.hasOwnProperty("Parent_Guardian_Name") || result.Parent_Guardian_Name.length === 0) {
        throw redirect(302, "/application-form")
    }

    if(!result.hasOwnProperty("Relationship") || result.Relationship.length === 0) {
        throw redirect(302, "/application-form")
    }
