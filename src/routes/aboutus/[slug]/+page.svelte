				...
				<ul class="space-y-2">
					{#each items as item}
						<li id={item} on:click={() => setActiveTabValue(item)}>
							<a
								href={`/aboutus/${item}`}
								class="flex items-center 2xl:justify-center p-2 3xs:p-1 text-base font-normal rounded-lg
								{$activeTab === item ? ' bg-white text-black ' : 'text-white'} "
							>
								<span class="ml-3 md:text-base 2xl:text-center  3xs:text-xs xs:text-sm"
