...
<article class="faculty-card" style={`--stagger: ${index * 70}ms; --primary-color: ${departmentColor.primary}; --light-bg: ${departmentColor.lightBg}; --badge-bg: ${departmentColor.badgeBg}; --badge-text: ${departmentColor.badgeText || '#ffffff'};`}>
	<header class="card-header">
		<div class="avatar-wrap" aria-hidden="true">
			<div class="avatar-inner">
				<img src={displayImage} alt={formattedName} loading="lazy" class:placeholder-avatar={!hasPhoto} class:female-avatar={female && !hasPhoto} />
			</div>
		</div>
		<div class="identity">
	...
	<footer class="card-footer">
		...
		<div class="meta-row">
			{#if faculty.email}
				<a href={gmailComposeUrl} target="_blank" rel="noopener noreferrer" class="meta-link email-link" title="Send email">
					<svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
						<path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/>
						<path d="M22 6l-10 7L2 6" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/>
					</svg>
				</a>
			{/if}

			{#if linkedInUrl}
				<a href={linkedInUrl} target="_blank" rel="noopener noreferrer" class="meta-link linkedin-link" title="Open LinkedIn profile">
...
<style>
	.faculty-card {
		...
		background:
			linear-gradient(180deg, color-mix(in srgb, var(--light-bg, rgba(224, 242, 254, 0.6)) 38%, #ffffff 62%), #ffffff 40%),
			#ffffff;
	...
	.avatar-inner {
		...
		height: 100%;
		border-radius: 50%;
		background: linear-gradient(145deg, color-mix(in srgb, var(--light-bg, rgba(224, 242, 254, 0.6)) 55%, #ffffff 45%), #f8fafc);
		display: grid;
		place-items: center;
